...
- [x] P31-T3: Integrate the appraisal router and validation gate into the live agent tick loop so LLM appraisal runs in-line for eligible ticks during actual simulation — Round round-0044 — Wire appraisal router into simulate_day_bridged for live LLM
- [x] P31-T4: Build structured audit logging that records per-tick path selection, LLM raw output, validation verdict, and fallback events for every appraisal invocation — Round round-0045 — Structured audit logging for per-tick LLM appraisal path sel
- [x] P31-T5: Verify that settlement and bookkeeping engines produce correct results when consuming LLM-sourced appraisal output, confirming contract compatibility end-to-end — Round round-0046 — Settlement engine contract compatibility tests for LLM-sourc
- [x] P31-T6: Run multi-day live simulation with LLM appraisal active for T1/T2/T4, audit fallback rates and output stability, and confirm no regression in deterministic-path ticks — Round round-0047 — Multi-day live simulation audit: LLM appraisal fallback rate
