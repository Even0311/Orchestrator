# Round Brief — round-0041 (attempt 1)
...
## Current Phase: P30: Offline / Shadow LLM Appraisal Validation
**Phase ID:** P30
**Phase Goal:** Validate LLM appraisal against the extracted appraisal seam without allowing it to drive the authoritative live path yet.
...
- P30-T1: Define the prompt schema and template contract for converting AppraisalInput into an LLM prompt and parsing the LLM response back into AppraisalOutput — Round round-0036 — Prompt schema and template contract for LLM appraisal
- P30-T2: Build a shadow appraisal runner that accepts AppraisalInput, invokes the LLM offline, and returns a parsed AppraisalOutput without touching the live path — Round round-0037 — Shadow appraisal runner: AppraisalInput → LLM → AppraisalOut
- P30-T3: Define acceptance rules that specify per-field comparison criteria between LLM-produced and deterministic AppraisalOutput, distinguishing structural validity from semantic quality — Round round-0038 — Per-field acceptance rules for LLM vs deterministic Appraisa
- P30-T4: Build a comparison harness that runs both deterministic and shadow LLM paths on the same AppraisalInput, produces a structured diff report, and flags acceptance-rule violations — Round round-0039 — Comparison harness: run both paths, diff, flag violations
- P30-T5: Execute shadow comparisons across representative T1/T2/T4 test scenarios, collect structured diff reports, and persist raw results for analysis — Round round-0040 — Offline shadow comparison execution: T1/T2/T4 scenarios with

## Selected Task
**Task Key:** P30-T6
**Description:** Classify observed failure modes from shadow results into categories, define required guardrails for each category, and document which failures are blocking vs acceptable for live handoff
...
- **round-0040** [PASSED] — Execute the comparison harness across a representative set of T1, T2, and T4 App
- **round-0039** [PASSED] — Build back/app/llm/comparison_harness.py that accepts an AppraisalInput, runs th
- **round-0038** [PASSED] — Create a module at back/app/llm/acceptance_rules.py that defines per-field compa
