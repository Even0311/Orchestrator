# Round Brief — round-0047 (attempt 1)
...
- P31-T4: Build structured audit logging that records per-tick path selection, LLM raw output, validation verdict, and fallback events for every appraisal invocation — Round round-0045 — Structured audit logging for per-tick LLM appraisal path sel
- P31-T5: Verify that settlement and bookkeeping engines produce correct results when consuming LLM-sourced appraisal output, confirming contract compatibility end-to-end — Round round-0046 — Settlement engine contract compatibility tests for LLM-sourc

## Selected Task
**Task Key:** P31-T6
**Description:** Run multi-day live simulation with LLM appraisal active for T1/T2/T4, audit fallback rates and output stability, and confirm no regression in deterministic-path ticks
...
- **round-0045** [PASSED] — Build a structured audit logging layer that captures, for every appraisal invoca
