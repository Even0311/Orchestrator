...
**ID:** round-0008  
**Objective:** Modify `build_signal_from_t4_relationship_tick()` to read `world.social_event` and evaluate it against narrow trigger conditions approved for Phase 26B, establishing the input seam for event-aware T4 activation. Additionally, resolve the import regression causing `test_appraisal_settlement.py` to fail on `from app.domain.enums import Rel`.

**Exact Scope:** IN: Adding logic to `build_signal_from_t4_relationship_tick()` to access `world.social_event` and check for qualifying events using narrow trigger conditions (specific event types/categories approved for Phase 26B), returning appropriate indicators when qualifying events are detected. Fixing the ImportError in `test_appraisal_settlement.py` at line 7 to ensure `Rel` can be imported from `app.domain.enums`. OUT: Defining detailed output signal shapes (covered in P26B-T2), modifying downstream residual creation gates, expanding event taxonomy beyond the approved narrow set, implementing routing based on `social_event.target_id`, changes to other tick bridges (T1/T2/T3/T5-T8), settlement/substrate modifications, LLM integration, refactoring unrelated test logic beyond the import fix.
...
- Must fix failing test: resolve ImportError for `Rel` in `test_appraisal_settlement.py`
- Must ensure full pytest suite passes (exit code 0)
...
- ImportError in `test_appraisal_settlement.py:7` resolved; `from app.domain.enums import Rel` executes successfully
- All existing tests in `test_appraisal_settlement.py` pass
...
- test that `test_appraisal_settlement.py` imports execute without ImportError
- test that all existing appraisal_settlement tests pass
...
- do not modify `test_appraisal_settlement.py` beyond fixing the import error
