...
**ID:** round-0009  
**Objective:** Modify build_signal_from_t4_relationship_tick() to read world.social_event and emit a minimal activation signal when narrow trigger conditions are met, establishing the first event-aware T4 seam. Must include working tests that verify the behavior and pass completely.

**Exact Scope:** IN: Modifying build_signal_from_t4_relationship_tick() to check world.social_event for qualifying narrow trigger conditions (e.g., specific negative event categories), returning a minimal AppraisalSignal when triggered, returning None when no qualifying event exists. Writing or updating tests to verify this behavior and ensuring they pass. OUT: Defining detailed output schema variations, modifying downstream settlement gates, expanding event taxonomy, implementing target_id routing, changing existing T1/T2 bridge logic.
...
- Must use python3 or explicit pytest path commands (avoid bare 'python' which may not exist in environment)
- Tests must verify actual behavior, not just assert True

## Acceptance Criteria
- Implementation produces observable git changes (files modified, not empty)
...
- All pytest tests pass (exit code 0) including new T4 social event detection tests — no 'python not found' errors

## Required Tests
- Test that T4 builder detects qualifying narrow social event and returns activation signal with correct minimal shape
